...
public class BezierUtils
{
	...
	/// <summary>
	/// 任意阶贝塞尔曲线，根据T值，计算贝塞尔曲线上面相对应的点
	/// </summary>
	/// <param name="t">插量值</param>
	/// <param name="PointList"></param>控制点列表
	/// <returns></returns>存储贝塞尔曲线点的数组
    private static Vector3 CalculateArbitrarilPowerBezierPoint(float t,Vector3 [] PointList)
    {
        float u= 1 - t;
        Vector3 p;
        for(i=0;i<PointList.Length;i++)
        {
			p += PointList[i]*Math.Pow(u,PointList.Length-1-i)*Math.Pow(t,i);
        }
		return p;
    }
    ...
    public static Vector3[] GetArbitrarilPowerBeizerList(Vector3[] PointList,int segmentNum)
    {
        Vector3[] path=new Vector3[segmentNum];
		for(int i=1;i<=segmentNum;i++)
        {
            float t = i / (float)segmentNum;
			Vector3 pixel = CalculateArbitrarilPowerBezierPoint(t, PointList);
            path[i - 1] = pixel;
            Debug.Log(path[i - 1]);     
        }
        return path;
